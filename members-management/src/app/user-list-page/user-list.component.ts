...
export class UserListComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.userCreatedSubcription = this.userService.userCreated.subscribe(
      (data) => {
        this.showSuccess();
      }
    );
    ...
    this.userService.getUsers().subscribe({
      complete: () => {},
      error: (err) => {
        this.fetchError(err.statusText, err.name);
      },
      next: (data) => {
  ...
  fetchError(heading: any, message: any) {
    this.messageService.add({
      severity: 'error',
      summary: heading,
      detail: message,
  ...
  userAlreadyExist() {
    this.messageService.add({
      ...
      detail: 'Enter a Unique Username',
    });
  }
